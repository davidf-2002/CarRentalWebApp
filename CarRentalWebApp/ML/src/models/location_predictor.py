...
class LocationPredictor:
    ...
    def predict_future_demand(self, future_dates: List[pd.Timestamp], locations: List[str], vehicle_types: List[str]) -> pd.DataFrame:
        ...
        try:
            ...
            # Add required columns with sensible defaults based on historical data
            pred_data['dropoff_date'] = pred_data['date'] + pd.Timedelta(days=1)
            pred_data['rental_duration'] = 1  # Default 1-day rental
            pred_data['rate.daily'] = 100  # Default daily rate
            pred_data['rating'] = 4.0  # Default rating
